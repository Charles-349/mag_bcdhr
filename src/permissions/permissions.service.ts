...
// CREATE PERMISSION
export const addPermissionService = async (data: { name: string; description?: string }) => {
  const existing = await db.query.permissions.findFirst({ where: eq(permissions.name, data.name) });
...
export const getPermissionByIdService = async (id: number) => {
  return await db.query.permissions.findFirst({ where: eq(permissions.id, id) });
};

// UPDATE PERMISSION
export const updatePermissionService = async (id: number, data: Partial<{ name: string; description?: string }>) => {
...
export const deletePermissionService = async (id: number) => {
  ...
  // Remove any rolePermission links
